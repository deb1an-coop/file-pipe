...
from app.models.task_models import (
    TaskResponse, TaskStatusResponse, TaskListResponse, CreateTaskRequest,
    TaskType, TaskStatus, DataProcessingRequest, FileProcessingRequest, 
    EmailTaskRequest
)
from app.tasks.background_tasks import (
    process_data_task, process_file_task, send_email_task, generate_report_task
)
    ...
    try:
        ...
        if result.state == 'PENDING':
            ...
            task_info.update({
                ...
                'started_at': datetime.fromisoformat(info['started_at']) if info.get('started_at') else datetime.utcnow()
            })
    ...
    try:
        ...
        if request.task_type == TaskType.DATA_PROCESSING:
            ...
            task = generate_report_task.delay(
                request.parameters.get('report_type', 'default'),
                request.parameters
            )
            task_id = task.id
